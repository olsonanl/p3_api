...
**`origin/alpha` is 21 commits AHEAD of `origin/master`, 0 behind.** Master can be
...
back — that was true then and is not now. Check with
`git rev-list --left-right --count origin/master...origin/alpha` rather than trusting either
sentence.

- `origin/master` = **`6397c6cf`**, `origin/alpha` = **`4f64b1d4`**.
...
- **PR #206** (`4f64b1d4`, merged 2026-09-03) put the 21 commits on alpha: the whole
  self-call elimination, the `ShardCursorStream` POST fix, and the `MinHeap` UTF-8 comparator
  fix.
...
Open PRs as of 2026-09-03: **#207** (alpha ← `olsonanl:perf/rql-terms-cache`, the `terms()`
`cache=false` + `Limiter` work), **#205** (master ← dependabot, fast-uri 3.1.5→3.1.7), and
**#151** (alpha ← `bugfix/private-structure`, `protein_structure` access control, open since
2026-03-05).
...
**Offline-suite baseline on `origin/alpha` is 351 passing / 1 failing** (measured 2026-09-03,
`npx mocha tests/test-util/ tests/test-join/ tests/test-distributed/`) — the known
`fastaHeaderFormatter` case. Earlier notes here said 350/1 and then 327/1; both were stale.
PR #207 adds 19 offline specs on top, so that branch reads **370 / 1**. The self-call work's
own specs all need a live API and a populated Solr, so none of them land in these three
suites — run those against `:23001` with the command in `PLAN_ELIMINATE_SELF_CALL.md`.
