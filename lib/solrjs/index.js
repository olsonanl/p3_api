...
module.exports = declare([EventEmitter], {
  ...
  query: function (query, options) {
	...
	const req = reqObj.request({
      ...
      // Optional request timeout. Without this a Solr connection that accepts
      // but never responds (e.g. a stale keepAlive socket the server has already
      // dropped) hangs until the OS tears down the TCP connection — observed as
      // ~166s stalls. setTimeout fires on inactivity; destroy() aborts the
      // socket so the awaiting caller gets a prompt error instead of hanging.
      const timeoutMs = (options && options.timeout) || this.timeout
      if (timeoutMs) {
