...
RQLQuery.prototype.toSolr = function (opts) {
  ...
  if (normalized.genome && normalized.genome.length > 0) {
    ...
    var joinFilter = parts.length === 1 ? parts[0] : `(${parts.join(' AND ')})`

    if (opts.collection === 'genome') {
      // When querying the genome collection itself, the genome() filter can be
      // applied directly as an fq — no cross-collection join needed since the
      // genome collection already has all the fields being filtered on.
      console.log(`[GenomeSelfJoin] inlining filter=${joinFilter} (no join needed for genome collection)`)
      sq += '&fq=' + joinFilter
    } else {
      var joinQuery = `&fq={!join method=crossCollection fromIndex=genome from=genome_id to=${joinToColumnName}}` + joinFilter
      console.log(`[CrossCollectionJoin] filter=${joinFilter} joinTo=${joinToColumnName} collection=${opts.collection || 'unknown'}`)
      sq += joinQuery
    }
  }

  // Emit {!terms} filter queries for terms() operator.
  //
  // cache=false is deliberate, and it is not a micro-optimization. Caching the
  // filter makes Solr materialize its DocSet up front and then drive iteration
  // from the *scored* query, checking each hit against that set. That is fine
  // when the id list is the only clause, but terms() vacates q=, so a query
  // that also carries an ordinary clause degrades badly:
  //
  //   in(feature_type,(...)) & in(feature_id,(6000 ids))
  //     -> q=(feature_type:(...)) AND (feature_id:(a OR b ...))   one boolean,
  //        Solr leads with the selective id clause
  //   in(feature_type,(...)) & terms(feature_id,(6000 ids))
  //     -> q=(feature_type:(...))  fq={!terms f=feature_id}...    Solr scores
  //        ~the whole core and filters afterwards
  //
  // Measured on the production cluster, 6000 ids plus feature_type:(mat_peptide
  // OR CDS), rows held constant (min-of-2 QTime): both-in() 2418 ms, cached
  // terms 3952 ms, terms with cache=false 676 ms. Uncached, the terms query is
  // an ordinary scorer in the intersection and — being far more selective —
  // leads it. Note the cached figure did not improve on a repeat of the same id
  // list: the cost is the scoring, not building the filter, so a filterCache hit
  // does not rescue it.
  //
  // Standalone (no co-occurring clause) the two are a wash: 33/31, 74/80,
  // 178/174 ms at n=100/500/1500, min-of-5 with a fresh id list per rep. So this
  // gives up nothing in the simple case, and it stops one-shot id lists from
  // evicting genuinely reusable entries from the filterCache.
  //
  // The remaining trade is a client that re-sends an identical terms() filter
  // while paging; that pays for the cache. No such caller exists today — the web
  // grid pages with in(), and terms() must be written explicitly by the client.
  if (normalized.terms && normalized.terms.length > 0) {
    normalized.terms.forEach(function (t) {
      sq += '&fq={!terms f=' + t.field + ' cache=false}' + t.values.join(',')
    })
  }
...
RQLQuery.prototype.normalize = function (options) {
  ...
  function normal (func, args) {
    ...
    if (func === 'sort' || func === 'select') {
      ...
      // terms(field,(val1,val2,...)) — store for {!terms} filter emission
      result.terms.push({ field: args[0], values: args[1] })
    } else if (func === 'genome') {
...
function queryToSolr (part, options) {
  ...
  if (part && part.name && part.args) {
    if (_handlerMap[part.name]) {
      return _handlerMap[part.name](part, options)
    }
    // An operator node (has name + args) with no handler is a malformed query,
    // e.g. "keyword(x)>(field,val)" parses to a bogus "keywordgt" node. Without
    // this guard it falls through to encodeValue and is serialized as a literal
    // value ("object:keywordgt(...)"), which Solr rejects with the opaque
    // "undefined field object". Fail fast with an actionable message instead;
    // RQLQueryParser's catch turns a thrown error into a sanitized 400.
    throw new Error('Query Syntax Error: unknown operator ' + part.name)
  }
...
var handlers = [
  ['and', function (query, options) {
    ...
    // Every child was consumed elsewhere (into fq/fl/rows/sort) — e.g.
    // and(genome(...),select(f),limit(5)), where genome() becomes an fq and
    // select/limit become fl/rows. Returning '()' here yields a literal `q=()`,
    // which Solr rejects with "Cannot parse '()'". Return empty so the caller's
    // `if (!sq) sq = '*:*'` guard applies.
    if (parts.length === 0) {
      return ''
    }
    if (parts.length === 1) {
  ...
  ['or', function (query, options) {
    ...
    // Same empty-group guard as 'and' above — '()' is not a parseable Solr query.
    if (parts.length === 0) {
      return ''
    }
    if (parts.length === 1) {
  ...
  // terms(field,(val1,val2,...)) — efficient Solr terms filter using hash set
  // Use instead of in() for large value lists (hundreds+)
  // Generates: &fq={!terms f=field cache=false}val1,val2,...
  // Handled in normalize(); this handler is a no-op during the walk phase.
  ['terms', function (query, options) {
  }],
